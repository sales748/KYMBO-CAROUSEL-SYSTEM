...
- **No stock photos, no icon fonts.** Every scene photo is generated by nanobananapro (image gen, Kymbo runs it) against the STYLE LINE in the scene's content JSON. Every mockup is drawn in HTML/CSS via `system/app.mjs` — no royalty-free imagery.
